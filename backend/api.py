...
def get_fashion_recommendations():
    ...
    try:
        ...
        # Clear old cached files to ensure fresh generation
        old_files = ['facial_features.json', 'llm_recommendations.json', 'multi_scraped_output.json']
        for old_file in old_files:
            try:
                if os.path.exists(old_file):
                    os.remove(old_file)
                    print(f"Cleared old file: {old_file}")
            except Exception as e:
        ...
        # Load fresh data generated by process_image_for_recommendations
        facial_features = {}
        llm_recommendations = []
        ...
        # Prepare response with fresh scraped results
        response_data = {
            'success': True,
            'user_info': {
                'height': height,
            ...
            'generation_timestamp': time.time()  # Add timestamp to show it's fresh
        }
